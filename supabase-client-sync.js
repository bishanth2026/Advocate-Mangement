...
(function(){
  ...
  function isCorruptClient(c){
    if(!c||!CASE_TYPES[String(c.name||'').trim().toLowerCase()])return false;
    var phone=String(c.phone||'').trim(),email=String(c.email||'').trim();
    return !/^\+?[0-9 ()-]{7,}$/.test(phone)||/^(os|cc|wp|op|cr|fc)(\s|\/|-|\d|$)/i.test(phone)||/^(os|cc|wp|op|cr|fc)(\s|\/|-|\d|$)/i.test(email);
  }
  ...
  async function list(){if(!ready())throw new Error('Cloud session is not ready');return (await window.ADCloudCRUD.list('clients',{order:'created_at',ascending:false})).map(normalize).filter(function(c){return !isCorruptClient(c);});}
  ...
  async function remove(id){
    ...
    var linkedCases=[];
    try{linkedCases=await window.ADCloudCRUD.list('cases',{filters:{client_id:id}});}catch(e){console.warn('[AdvocateDesk] linked case lookup skipped:',e.message||e);}
    for(var i=0;i<linkedCases.length;i++){if(linkedCases[i]&&validId(linkedCases[i].id))await window.ADCloudCRUD.remove('cases',linkedCases[i].id);}
  ...
  function hook(){
    ...
    for(var i=0;i<buttons.length;i++){
      (function(b){var t=(b.textContent||'').trim().toLowerCase();
        if(/^(save client|save|update client|update)$/.test(t)){b.dataset.clientCloudHook='1';b.addEventListener('click',function(){if(!ready())return;var c={name:value(m,['[name="name"]','input[placeholder*="name" i]']),phone:value(m,['[name="phone"]','input[type="tel"]']),email:value(m,['[name="email"]','input[type="email"]']),address:value(m,['[name="address"]']),notes:value(m,['[name="notes"]','textarea']),status:value(m,['[name="status"]'])||'Active'};var id=clientId(m);save(c,id).then(function(row){var s=read();s.clients=Array.isArray(s.clients)?s.clients:[];var n=normalize(Object.assign({},c,row||{}, {id:(row&&row.id)||id||('cloud-'+Date.now())}));var ix=s.clients.findIndex(function(x){return String(x.id)===String(n.id);});if(ix>=0)s.clients[ix]=n;else s.clients.unshift(n);write(s);alert('Client saved to Supabase.');location.reload();}).catch(function(e){alert('Could not save client: '+(e.message||e));});},true);}
        if(/^(delete client|delete|remove client|remove)$/.test(t)){b.dataset.clientCloudHook='1';b.addEventListener('click',function(){var id=clientId(m);if(!validId(id)||!ready())return;if(!confirm('Delete this client and its linked cases from Supabase? Unlinked cases will be preserved.'))return;remove(id).then(function(){alert('Client deleted. Linked cases were also removed.');location.reload();}).catch(function(e){alert('Could not delete client: '+(e.message||e));});},true);}
      })(buttons[i]);
    }
