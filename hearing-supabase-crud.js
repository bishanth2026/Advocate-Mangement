...
(function(){
  ...
  function payload(m){
    return {
      case_id:val(m,'Case')||val(m,'Case Number')||null,
      hearing_date:val(m,'Hearing Date')||val(m,'Date')||null,
      purpose:val(m,'Purpose')||val(m,'Stage'),
      court_room:val(m,'Court Room')||val(m,'Court'),
      status:val(m,'Status')||'Scheduled',
      notes:[val(m,'Hearing Time')?'Time: '+val(m,'Hearing Time'):'',val(m,'Notes')].filter(Boolean).join('\n')
    };
  }
