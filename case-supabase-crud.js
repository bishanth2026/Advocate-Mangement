...
(function(){
  ...
  async function saveCase(m){
    ...
    try{if(id)await window.ADCloudCRUD.update('cases',id,p);else await window.ADCloudCRUD.insert('cases',p);alert('Case saved securely to Supabase.');window.location.reload();}
  ...
  function enhanceDelete(m){
    ...
    del.addEventListener('click',async function(){var id=findExistingId(m);if(!id){alert('This case is not yet saved in Supabase.');return;}if(!confirm('Delete this case permanently?'))return;try{await window.ADCloudCRUD.remove('cases',id);alert('Case deleted.');window.location.reload();}catch(e){alert('Could not delete case: '+(e.message||e));}});
    saveButton.parentElement.insertBefore(del,saveButton);
  }
  function boot(){if(active)return;active=true;new MutationObserver(function(){var m=modal();if(m&&isCaseModal(m))enhanceDelete(m);}).observe(document.body,{childList:true,subtree:true});document.addEventListener('click',function(e){var b=e.target.closest&&e.target.closest('button'),m=modal();if(!b||!m||!isCaseModal(m)||!ready())return;if((b.textContent||'').trim().toLowerCase()==='save'){e.preventDefault();e.stopImmediatePropagation();saveCase(m);}},true);}
