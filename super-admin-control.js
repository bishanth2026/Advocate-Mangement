...
(function(){
  ...
  function ensureNav(){
    ...
    btn.setAttribute('aria-label','Central Control');
  }
  ...
  function render(){
    ...
    if(!content)return;
    const a=list();
    content.innerHTML=(window.P1&&P1.layout?P1.layout('Admin Accounts','Super Admin control of administrator login accounts',`<button class="primary" onclick="adAdminAdd()">＋ Create Admin</button>`):`<div class="page-title"><div><h1>Admin Accounts</h1><p>Super Admin control of administrator login accounts</p></div><button class="primary" onclick="adAdminAdd()">＋ Create Admin</button></div>`)+
  ...
  window.adAdminReset=function(i){const a=list(),x=a[i];if(!x)return;const p=prompt('Enter the new password for '+x.email+':','demo123');if(p===null)return;if(!p.trim()||p.trim().length<4)return alert('Password must be at least 4 characters.');x.password=p.trim();save(a);alert('Admin password reset successfully.')};
