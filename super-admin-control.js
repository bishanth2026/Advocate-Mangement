...
(function(){
  ...
  function ensureNav(){
    ...
    if(!wrap){
      ...
      wrap.innerHTML='<button class="nav-item super-nav-active" data-super-page="overview"><span class="nav-icon">⌂</span><span>Overview</span></button><button class="nav-item" data-super-page="organizations"><span class="nav-icon">▦</span><span>Organizations</span></button><button class="nav-item" data-super-page="admins"><span class="nav-icon">⚙</span><span>Admin Accounts</span></button><button class="nav-item" data-super-page="settings"><span class="nav-icon">⚙</span><span>Settings</span></button>';
      const nav=document.querySelector('#nav');if(nav)nav.insertAdjacentElement('afterend',wrap);
    }
    wrap.querySelectorAll('.nav-item').forEach(function(b){b.style.display='flex'});
    const bottom=document.querySelector('.sidebar-bottom');if(bottom){const profile=bottom.querySelector('.profile-mini');if(profile)profile.style.display='flex'}
    if(window.ADSuperSettings&&ADSuperSettings.ensureNav)ADSuperSettings.ensureNav();
  }
  function shell(title,sub,body){return `<div class="page-title"><div><h1>${title}</h1><p>${sub}</p></div></div>${body}`}

  /* The normal dashboard/finance layers can still run after the Super Admin control
     plane is rendered. Remove any customer-finance cards that those layers inject. */
  function removeFinanceCards(){
    if(!isSuper())return;
    const content=document.getElementById('content');if(!content)return;
    const financeTerms=['Agreed Fees','Additional Charges','Amount Received','Outstanding Fees','Balance Fees','Fees Received','General Transactions','Expenses','Net Income'];
    content.querySelectorAll('.stat,.card,.dashboard-card,.metric-card').forEach(function(card){
      const text=(card.innerText||card.textContent||'').trim();
      if(financeTerms.some(function(term){return text.indexOf(term)!==-1}))card.remove();
    });
    content.querySelectorAll('h1,h2,h3,h4,p,div').forEach(function(el){
      const t=(el.innerText||el.textContent||'').trim();
      if((t==='Agreed Fees'||t==='Additional Charges'||t==='Amount Received'||t==='Outstanding Fees')&&el.closest('.stat,.card,.dashboard-card,.metric-card'))el.closest('.stat,.card,.dashboard-card,.metric-card').remove();
    });
  }
  function installFinanceGuard(){
    if(!isSuper()||window.__superFinanceGuard)return;
    window.__superFinanceGuard=true;
    const content=document.getElementById('content');if(!content)return;
    const observer=new MutationObserver(function(){removeFinanceCards()});
    observer.observe(content,{childList:true,subtree:true});
    window.__superFinanceObserver=observer;
    setTimeout(removeFinanceCards,0);setTimeout(removeFinanceCards,100);setTimeout(removeFinanceCards,500);setTimeout(removeFinanceCards,1200);
  }

  function overview(){
    ensureNav();installFinanceGuard();const a=list(),active=a.filter(x=>x.status==='Active').length,disabled=a.filter(x=>x.status!=='Active').length,orgs=[...new Set(a.map(x=>String(x.organization||'').trim()).filter(Boolean))];
    const content=document.getElementById('content');if(!content)return;
    content.innerHTML=shell('Super Admin Control','Central platform overview — customer law-office workspaces are kept separate.',`<div class="notice"><strong>Super Admin mode</strong> — You are in the platform control area. Customer cases, clients, hearings, documents and finance are not shown here.</div><div class="cards"><div class="stat"><div class="stat-top">Organizations</div><div class="stat-value">${orgs.length}</div><div class="stat-foot">Law offices managed</div></div><div class="stat"><div class="stat-top">Admin Accounts</div><div class="stat-value">${a.length}</div><div class="stat-foot">Administrator accounts</div></div><div class="stat"><div class="stat-top">Active Admins</div><div class="stat-value">${active}</div><div class="stat-foot">Currently enabled</div></div><div class="stat"><div class="stat-top">Disabled Accounts</div><div class="stat-value">${disabled}</div><div class="stat-foot">Access disabled</div></div></div><div class="grid-2"><div class="panel"><div class="panel-head"><div><h3>Central Control</h3><span class="muted">Manage platform access without entering a customer's workspace.</span></div></div><div class="panel-body" style="padding:14px"><div class="quick-grid"><button class="quick" onclick="ADSuper.go('organizations')"><strong>▦ Organizations</strong><small>View law offices assigned to Admin accounts</small></button><button class="quick" onclick="ADSuper.go('admins')"><strong>⚙ Admin Accounts</strong><small>Create, edit, enable, disable and reset access</small></button><button class="quick" onclick="ADSuper.go('settings')"><strong>⚙ Settings</strong><small>Change Super Admin account and security settings</small></button></div></div></div><div class="panel"><div class="panel-head"><div><h3>Platform Access</h3><span class="muted">Security boundary</span></div></div><div class="panel-body" style="padding:14px"><p style="margin:0 0 8px">Super Admin controls the platform and customer Admin access.</p><p class="muted" style="margin:0">Customer case and finance data remains inside the Admin workspace. Production authentication and organization isolation will be enforced with Supabase Auth and RLS.</p></div></div></div>`);setActive('overview');removeFinanceCards();
  }
  function organizations(){ensureNav();const a=list(),map={};a.forEach(x=>{const o=String(x.organization||'Unassigned').trim()||'Unassigned';if(!map[o])map[o]={name:o,total:0,active:0};map[o].total++;if(x.status==='Active')map[o].active++});const rows=Object.values(map).map(o=>`<tr><td><strong>${esc(o.name)}</strong></td><td>${o.total}</td><td>${o.active}</td><td>${o.total-o.active}</td></tr>`).join('');const content=document.getElementById('content');if(!content)return;content.innerHTML=shell('Organizations','Law offices represented by the Admin accounts on this browser.',`<div class="notice">Organization assignment is managed from Admin Accounts. Full cloud organization management will be connected with Supabase.</div><div class="panel"><div class="panel-head"><div><h3>Law Offices</h3><span class="muted">Overview of current Admin assignments</span></div><button class="secondary" onclick="ADSuper.go('admins')">Manage Admins</button></div><div class="p1-fee-table-wrap"><table><thead><tr><th>Law Office / Organization</th><th>Total Admins</th><th>Active</th><th>Disabled</th></tr></thead><tbody>${rows||'<tr><td colspan="4">No organizations yet.</td></tr>'}</tbody></table></div></div>`);setActive('organizations')}
  function admins(){ensureNav();const a=list(),content=document.getElementById('content');if(!content)return;content.innerHTML=shell('Admin Accounts','Control administrator access for each customer law office.',`<div class="notice">Only Super Admin can create, edit, disable, enable or reset Admin accounts. Passwords are never displayed.</div><div class="panel"><div class="panel-head"><div><h3>Administrators</h3><span class="muted">Manage law-office administrator access.</span></div><button class="primary" onclick="adAdminAdd()">＋ Create Admin</button></div><div class="p1-fee-table-wrap"><table><thead><tr><th>Admin</th><th>Email / Username</th><th>Law Office</th><th>Status</th><th>Actions</th></tr></thead><tbody>${a.map((x,i)=>`<tr><td><strong>${esc(x.name)}</strong></td><td>${esc(x.email)}</td><td>${esc(x.organization||'—')}</td><td><span class="badge ${x.status==='Active'?'green':'red'}">${esc(x.status)}</span></td><td><div style="display:flex;gap:6px;flex-wrap:wrap"><button class="secondary" onclick="adAdminEdit(${i})">Edit</button><button class="secondary" onclick="adAdminToggle(${i})">${x.status==='Active'?'Disable':'Enable'}</button><button class="secondary" onclick="adAdminReset(${i})">Reset Password</button></div></td></tr>`).join('')}</tbody></table></div></div>`);setActive('admins')}
  ...
  function go(page){if(!isSuper())return;if(page==='organizations')organizations();else if(page==='admins')admins();else if(page==='settings'&&window.ADSuperSettings&&ADSuperSettings.settings)ADSuperSettings.settings();else overview()}
  ...
  function boot(){if(!isSuper())return;list();ensureNav();installFinanceGuard();const sidebar=document.querySelector('.sidebar');if(sidebar){sidebar.style.overflowY='auto';sidebar.style.overflowX='hidden'}const navHandler=function(e){const n=e.target.closest&&e.target.closest('#super-control-nav .nav-item');if(n&&isSuper()){e.preventDefault();e.stopImmediatePropagation();go(n.getAttribute('data-super-page'))}};document.addEventListener('click',navHandler,true);setTimeout(function(){ensureNav();overview()},150);setTimeout(ensureNav,600);setTimeout(removeFinanceCards,1500)}
