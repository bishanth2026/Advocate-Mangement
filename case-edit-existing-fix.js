...
(function(){
  ...
  function findClientValue(select, value){
    ...
    const option=Array.from(select.options||[]).find(function(o){
      return String(o.value||'').trim().toLowerCase()===wanted || String(o.textContent||'').trim().toLowerCase()===wanted || String(o.dataset.name||'').trim().toLowerCase()===wanted;
    });
  ...
  function hydrateCase(item){
    if(!item)return;
    setValue('f1',item.number||'');
    setValue('f2',item.title||'');
    setValue('f4',item.court||'');
    setValue('f5',item.next||item.nextHearing||'');
    setValue('f6',item.type||'Civil');
    setValue('f7',item.status||'Active');

    const number=String(item.number||'').trim().match(/^([A-Za-z]+)\s*(.*)$/);
    const prefix=document.getElementById('caseNumberPrefix');
    const numberInput=document.getElementById('caseNumberValue');
    if(number && prefix && numberInput){
      prefix.value=number[1].toUpperCase();
      numberInput.value=number[2]||'';
    ...
    if(clientsSelect && clientsSelect.multiple){
      Array.from(clientsSelect.options).forEach(function(o){o.selected=ids.map(String).includes(String(o.value));});
    ...
    findClientValue(document.getElementById('casePetitioner'),petitioner);
    findClientValue(document.getElementById('caseRespondent'),respondent);
  }
  function install(){
    ...
    function fixed(type,index){
      ...
      const list=data && data[collection(type)];
      const item=Array.isArray(list)?list[index]:null;
      original.apply(this,arguments);
      if(type==='case' && item){
        window.setTimeout(function(){hydrateCase(item);},0);
        window.setTimeout(function(){hydrateCase(item);},80);
      }
    }
    fixed.__caseEditFixed=true;
    window.openEditModal=fixed;
  }
