...
(function () {
  ...
  function makeCourtTypeahead() {
    document.querySelectorAll('.field select').forEach(function (select) {
      ...
      input.autocomplete = 'off'; input.placeholder = 'Type to search court...';
      ...
      Array.from(select.options).forEach(function (option) {
        ...
        if (!Array.from(datalist.options).some(function (x) { return x.value === option.text; })) {
          var item = document.createElement('option'); item.value = option.text; datalist.appendChild(item);
        }
      });
      function syncCourt() {
      ...
      input.addEventListener('input', syncCourt); input.addEventListener('change', syncCourt);
      select.style.display = 'none'; select.dataset.courtTypeahead = '1'; select.insertAdjacentElement('afterend', input);
    });
  }
  ...
  document.addEventListener('click', function (event) {
    ...
    if (menu || overlayHit) {
      var sidebar = document.querySelector('.sidebar'); var overlay = document.getElementById('mobileOverlay');
      if (sidebar && overlay) { var open = !!menu && !sidebar.classList.contains('open'); sidebar.classList.toggle('open', open); overlay.classList.toggle('show', open); overlay.setAttribute('aria-hidden', open ? 'false' : 'true'); }
    }
  ...
  var observer = new MutationObserver(function () { makeCourtTypeahead(); });
  ...
  function fixDashboard() {
    ...
    if (title && /Good morning|Good afternoon|Good evening/.test(title.textContent)) title.textContent = greeting + ', Advocate';
    ...
    try {
      ...
      hearings = stored && Array.isArray(stored.hearings) ? stored.hearings : [];
    } catch (error) { hearings = []; }
    var start = new Date(); start.setHours(0, 0, 0, 0);
    var end = new Date(start); end.setDate(end.getDate() + 30);
    var upcoming = hearings.filter(function (hearing) {
      if (!hearing || !hearing.date) return false;
      var date = new Date(String(hearing.date).slice(0, 10) + 'T00:00:00');
      return !isNaN(date.getTime()) && date >= start && date <= end;
    });
    var cards = content.querySelectorAll('.stat');
    if (cards.length > 1) {
      var value = cards[1].querySelector('.stat-value');
      if (value) value.textContent = upcoming.length;
    }
  }

  var dashboardObserver = new MutationObserver(function () { window.setTimeout(fixDashboard, 0); });
  dashboardObserver.observe(document.getElementById('content') || document.body, { childList: true, subtree: true });
  window.setTimeout(fixDashboard, 0);
  window.setTimeout(fixDashboard, 250);
