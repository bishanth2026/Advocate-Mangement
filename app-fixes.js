...
(function () {
  ...
  function enhanceHearingModal() {
    ...
    var signature = cases.map(function (c) { return c.id + ':' + (c.number || '') + ':' + (c.client || ''); }).join('|');
    if (caseControl.dataset.caseSignature !== signature) {
      ...
      caseControl.dataset.caseSignature = signature;
    }
  ...
  function fixDashboardDateAndCount() {
    ...
    if (title && title.textContent !== greeting + ', Advocate') title.textContent = greeting + ', Advocate';
    var subtitle = content.querySelector('.page-title p');
    var dateText = now.toLocaleDateString('en-IN', { weekday: 'long', day: '2-digit', month: 'long', year: 'numeric' }) + ' • Demo Workspace';
    if (subtitle && subtitle.textContent !== dateText) subtitle.textContent = dateText;
    ...
    if (cards[1]) { var value = cards[1].querySelector('.stat-value'); if (value && value.textContent !== String(count)) value.textContent = count; }
  }
  function runFixes() { makeCourtTypeahead(); hidePastHearings(); enhanceHearingModal(); fixDashboardDateAndCount(); }
  document.addEventListener('click', function (event) {
    var item = event.target.closest && event.target.closest('[data-page]');
    if (item && !item.disabled) { item.classList.add('tap-active'); window.setTimeout(function () { item.classList.remove('tap-active'); }, 180); closeMobileNav(); window.setTimeout(runFixes, 50); }
  ...
  runFixes();
  window.setInterval(runFixes, 1500);
  window.addEventListener('hashchange', function () { window.setTimeout(runFixes, 50); });
