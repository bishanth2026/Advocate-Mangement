(function(){
  ...
  window.calendar=function(){
    ...
    for(let day=1;day<=days;day++){
      const key=year+'-'+pad(month+1)+'-'+pad(day),list=byDate[key]||[],today=key===new Date().toISOString().slice(0,10);
      cells+=`<div class="calendar-cell ${today?'today':''}"><div class="calendar-day">${day}</div>${list.slice(0,3).map(e=>`<div class="calendar-event ${e.kind}" title="${String(e.title+' — '+e.meta).replace(/"/g,'&quot;')}">${e.title}</div>`).join('')}${list.length>3?`<div class="calendar-more">+${list.length-3} more</div>`:''}</div>`;
    }
  ...
  window.calendarPrev=()=>{if(!Number.isInteger(window.calendarMonth)){const d=new Date();window.calendarYear=d.getFullYear();window.calendarMonth=d.getMonth();}window.calendarMonth--;if(window.calendarMonth<0){window.calendarMonth=11;window.calendarYear--;}window.calendar();};
  window.calendarNext=()=>{if(!Number.isInteger(window.calendarMonth)){const d=new Date();window.calendarYear=d.getFullYear();window.calendarMonth=d.getMonth();}window.calendarMonth++;if(window.calendarMonth>11){window.calendarMonth=0;window.calendarYear++;}window.calendar();};
  ...
  document.addEventListener('click',function(e){
    ...
    if(button){e.preventDefault();e.stopImmediatePropagation();document.querySelectorAll('.nav-item').forEach(x=>x.classList.toggle('active',x===button));window.calendar();}
  },true);
