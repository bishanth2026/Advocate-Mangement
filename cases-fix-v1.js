(function(){
  ...
  function form(type,index){
    ...
    const options=caseTypes.map(t=>'<option value="'+esc(t)+'"'+(item.caseType===t?' selected':'')+'>'+esc(t)+'</option>').join('');
  ...
  window.casesFixSave=(type,index)=>{
    ...
    if(!caseType){alert('Please select the case type.');return;}
    ...
    const record={...(old||{}),type,name,caseType,caseNumber:document.getElementById('partyNumber').value.trim(),court:document.getElementById('partyCourt').value.trim(),phone:document.getElementById('partyPhone').value.trim(),email:document.getElementById('partyEmail').value.trim(),address:document.getElementById('partyAddress').value.trim(),details:document.getElementById('partyDetails').value.trim(),updatedAt:new Date().toISOString()};
