...
function boot(){document.addEventListener('click',e=>{const n=e.target.closest?.('.nav-item[data-page]');if(!n)return;const p=n.dataset.page;if(['finance','reports','settings','documents','calendar'].includes(p)){e.preventDefault();e.stopImmediatePropagation();P1.nav(p);if(p==='finance')window.p1Finance();if(p==='reports')window.p1Reports();if(p==='settings')window.p1Settings();if(p==='documents')window.p1Documents();if(p==='calendar')window.p1Calendar()}},true)}
