(function(){
  window.ADAuth={
    ...
    logout:function(){localStorage.removeItem('advocateDeskAuth');window.location.href='app.html'},
    require:function(){
      var a=this.get();
      if(!a){
        a={role:'admin',name:'Advocate Admin',email:'admin@localhost',loginAt:new Date().toISOString()};
        localStorage.setItem('advocateDeskAuth',JSON.stringify(a));
      }
      return a;
    }
  };
