(function(){
  ...
  function installLogoutMenu(){
    ...
    chip.setAttribute('data-logout-menu-ready','1');
    chip.type='button';
    ...
    menu.style.cssText='position:absolute;right:0;top:calc(100% + 8px);min-width:190px;background:#fff;border:1px solid #e5e7eb;border-radius:10px;box-shadow:0 12px 30px rgba(15,23,42,.14);padding:6px;z-index:2000;display:none;';
    ...
    function toggle(e){
      ...
      if(open)close();
      else{menu.style.display='block';chip.setAttribute('aria-expanded','true')}
    }

    chip.addEventListener('click',toggle);
    chip.addEventListener('touchend',function(e){e.preventDefault();toggle(e)},{passive:false});
    menu.querySelector('[data-account-logout]').addEventListener('click',function(e){
      e.preventDefault();
      e.stopPropagation();
      window.ADAuth.logout();
    });
  ...
  if(document.readyState==='loading')document.addEventListener('DOMContentLoaded',installLogoutMenu,{once:true});
  else installLogoutMenu();
})();
